...
my ($gap, $genome, $shuffle, $intersect, $outfile) = @ARGV;
die "usage: <gap file> <genome file> <file to be shuffled> <file to be intersected> <outfile name>\n$warning" unless @ARGV;
...
#	my $cmd = "bedtools shuffle -chrom -excl ../../gaps_hg19.bed -i ../common_late_origins_ENCODE.bed -g ../hg19.genome > temp_$i ; bedtools intersect -wo -a temp_$i -b ../../../../region_metaplots/cpg_islands_all_parsed.bed | awk '{print \$7}' | awk '{sum+=\$1} END {print sum}' >> 1mil_simulation_cpg_late.txt ; rm temp_$i";
	my $cmd = "bedtools shuffle -chrom -excl $gap -i $shuffle -g $genome > temp_$i ; bedtools intersect -a temp_$i -b $intersect |  awk '{print \$3 - \$2}' | awk '{sum+=\$1} END {print sum}' >> $outfile; rm temp_$i";
	$Q->enqueue($cmd);
}
$Q->end;
