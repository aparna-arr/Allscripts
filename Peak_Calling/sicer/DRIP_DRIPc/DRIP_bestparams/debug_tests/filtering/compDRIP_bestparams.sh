...
#FIXME uncomment when correct best params for dripc files are found. 
#sh $SICER_DIR/SICER-rb.sh . $BED1 ./temp1 hg19 1 <WINDOW> 150 0.74 <GAP> <LARGE_E>
#sh $SICER_DIR/SICER-rb.sh . $BED2 ./temp2 hg19 1 <WINDOW> 150 0.74 <GAP> <LARGE_E>
 ...
 do
   ...
   for i in {50..400..50} #window size
    ...
    # for no filtering by score uncomment below
           #bedtools intersect -u -a DRIP_temp/*.scoreisland -b temp1/*.scoreisland > intersect_outfile_DRIPfile1.bed
           #bedtools intersect -u -a DRIP_temp/*.scoreisland -b temp2/*.scoreisland > intersect_outfile_DRIPfile2.bed
    ################################
    # For filtering by score       #
    ################################
    # comment if no filtering by score
           bedtools intersect -u -a DRIP_temp/*.scoreisland -b temp1/*.scoreisland > intersect_outfile_DRIPfile1.tmp
           bedtools intersect -u -a DRIP_temp/*.scoreisland -b temp2/*.scoreisland > intersect_outfile_DRIPfile2.tmp
         for file in intersect*.tmp
          do
            while read line
              do
                num=`echo ${line##* }`
                ...
                if [ $num -gt 150 ] #score cutoff
                  then
                    echo $line >> $file.bed
                    #FIXME all intersect outfiles will be .tmp.bed
                fi
              done < $file
            rm $file
          done
      ################################
           int_count_1=`cat intersect_outfile_DRIPfile1.tmp.bed | wc -l`           
           int_count_2=`cat intersect_outfile_DRIPfile2.tmp.bed | wc -l`           
           rm intersect_outfile*.bed
           DRIP_peaks=`cat DRIP_temp/*.scoreisland | wc -l`
#TODO continue modifying script from here
#for loop for 2 files? Would be cleaner but much harder
