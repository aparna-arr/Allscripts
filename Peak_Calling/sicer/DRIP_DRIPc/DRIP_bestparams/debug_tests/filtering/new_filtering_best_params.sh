...
mkdir temp1
mkdir temp2
echo -e "E-value\tWindow_Size\tGap_Size\tNum_Intersects\tFile1_peaks\tFile2_peaks\tPercent_Overlap" > summary_file
   ...
   for i in {50..400..50} #window size
    ...
    # comment if no filtering by score
      ...
      ################################
           ...
           peaks1=`cat temp1/*.scoreisland.NEW | wc -l`
           peaks2=`cat temp2/*.scoreisland.NEW | wc -l`
           less_peaks=$peaks1
           if [ $peaks2 -lt $peaks1 ]
             then
               less_peaks=$peaks2
           ...
           rm temp1/*
           rm temp2/*
           echo -e "$LARGE_E\t$i\t$gap\t$int_count\t$peaks1\t$peaks2\t$percent_overlap" >> summary_file
         done
     done
