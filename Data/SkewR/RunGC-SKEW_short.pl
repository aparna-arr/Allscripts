...
my ($seqFile, $modelFile, $threshold, $threads, $projName, $length) = StochHMMToBed::check_sanity_short();
...
convert_report($projName, $length, $threshold, @splitFastaName);
...
sub convert_report {
	my ($projName, $length, $threshold, @splitFastaName) = @_;
	#my @states = ("C_SKEW", "G_SKEW");
	#my %colors = (C_SKEW => "0,0,255", G_SKEW => "255,0,0");
	my @states = ("P", "N");
	my %colors = (N => "0,0,255", P => "255,0,0");
