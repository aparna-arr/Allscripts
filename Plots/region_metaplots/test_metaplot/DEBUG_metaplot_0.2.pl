  ...
  foreach my $chrom (keys %chromosomes) {
    ...
    for ($i = 0; $i < @bedfiles ; $i ++) {
      ...
      while (<BED>) {
        ...
        $num ++ ; #num increments so each line has its own array(hash)
      }
    ...
    while(<WIG>) {
      ...
      else 
      {
        ...
        for ($n = $pos ; $n <= $pos + $span ; $n++) {
          $vals2{$n} = $val ; # each pos between $pos and $pos + $span equals the same value (which makes sense)
        }
      }
    }
    close WIG ;
...
######## ERROR HERE SOMEWHERE ################
      for(my $num2 = 0 ; $num2 < @{ $chrs[$bedfile] } ; $num2++ ) {
        ...
        my $m ;
        $x = 1 ; #don't need
        my $k = 1 ; #why initialized?
        my $l = $end - $start ; #should be + 1
...
#              $results[$bedfile][$k]{count} ++ ;
              $results[$bedfile][$m - $start]{height} += $vals2{$m} ; #FIX data struct
              $results[$bedfile][$m - $start]{count} ++ ;
            } # if val exists
...
########### ERROR END #################
#      my $test = $end - $start ;
#      print "end minus start $test\n" ;
#      print "x is $x for num2 : $num2\n" ;
      } #for num2
#    print "x is $x for bedfile\n" ;
    } # for bedfile
...
sub print_results {
  ...
  my $head = join("\t", @names) ;
  print OUT "bp\t$head\n" ;
  my $h = 0 - ($x / 2) ; #should not be from x, but from user input

  for (my $i = 1 ; $i < $x ; $i++) { #why not start i from 0 ? once k is fixed
    print OUT $h + $i, "\t" ;
