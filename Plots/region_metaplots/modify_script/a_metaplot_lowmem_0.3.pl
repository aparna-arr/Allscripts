...
sub get_input {
  ...
  validate(\@bedfiles, \@names, \$wigfile) ;
...
sub validate {
  ...
  if (scalar(@names) != scalar(@beds))
  {
    die "Error! # names != # beds! See usage.\n" ;
  }
      ...
      for(my $num2 = 0 ; $num2 < @{ $chrs[$bedfile] } ; $num2++ ) {
        ...
        for ($m = $start ; $m <= $end ; $m++) {
          ...
          if ($strand eq '+') {
            ...
            if (exists($vals{$m})) {
              $results[$bedfile][$k]{height} += $vals{$m} ; #FIX data struct
          ...
          if ($strand eq '-') {
            ...
            if (exists($vals{$m})) {
              $results[$bedfile][$l]{height} += $vals{$m} ; #FIX data struct
...
#      print "for loop 2.1 : chr (num2) \n" ;
    } # for bedfile
#    print "for loop 2 : bed file process\n" ;
